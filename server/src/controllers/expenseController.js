...
export const createExpense = async (req, res, next) => {
  try {
    ...
    const budgetCheck = await pool.query(
      'SELECT monthly_limit::float AS monthly_limit FROM budgets WHERE id = $1',
      [budgetId]
    )
    ...
    const totalQuery = 'SELECT COALESCE(SUM(amount), 0)::float AS total_spent FROM expenses WHERE budget_id = $1'
...
export const getExpensesForBudget = async (req, res, next) => {
  try {
    ...
    const budgetCheck = await pool.query('SELECT id FROM budgets WHERE id = $1', [budgetId])
    if (budgetCheck.rows.length === 0) {
      return sendError(res, 'Budget not found', 404)
    }
...
export const updateExpense = async (req, res, next) => {
  try {
    ...
    const cleanDescription = description !== undefined ? (description ? String(description).trim() : null) : undefined

    // Check expense existence
    const existingCheck = await pool.query('SELECT * FROM expenses WHERE id = $1', [expenseId])
    if (existingCheck.rows.length === 0) {
      return sendError(res, 'Expense not found', 404)
    }

    const currentExpense = existingCheck.rows[0]
    const finalAmount = numericAmount !== null ? numericAmount : currentExpense.amount
    const finalDescription = cleanDescription !== undefined ? cleanDescription : currentExpense.description
    ...
    const updateQuery = `
      ...
      RETURNING id, budget_id, amount::float AS amount, description, date, created_at
    `
    const result = await pool.query(updateQuery, [finalAmount, finalDescription, finalDate, expenseId])
...
export const deleteExpense = async (req, res, next) => {
  try {
    ...
    const deleteQuery = 'DELETE FROM expenses WHERE id = $1 RETURNING id, budget_id'
