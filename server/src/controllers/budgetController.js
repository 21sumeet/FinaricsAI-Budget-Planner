...
const isValidMonthFormat = (monthStr) => /^\d{4}-(0[1-9]|1[0-2])$/.test(monthStr)
...
export const createBudget = async (req, res, next) => {
  try {
    ...
    if (!month || !isValidMonthFormat(month)) {
      return sendError(res, 'Month must be in YYYY-MM format (e.g. 2026-09)', 400)
    }
    ...
    const result = await pool.query(
      ...
      [category.trim(), numericLimit, month]
    )
...
export const updateBudget = async (req, res, next) => {
  try {
    ...
    const updateQuery = `
      ...
      RETURNING id
    `
    const result = await pool.query(updateQuery, [category.trim(), numericLimit, month, budgetId])
...
export const deleteBudget = async (req, res, next) => {
  try {
    ...
    const deleteQuery = 'DELETE FROM budgets WHERE id = $1 RETURNING id'
    const result = await pool.query(deleteQuery, [budgetId])

    if (result.rows.length === 0) {
      return sendError(res, 'Budget not found', 404)
    }

    return sendSuccess(res, { message: 'Budget deleted successfully', id: budgetId })
  } catch (error) {
    next(error)
  }
}
